...
const SENSITIVE_FIELD_RE =
  /^(authorization|cookie|set-cookie|x-api-key|api-key|token|access_token|refresh_token|secret|secretkey|password|passwd|privatekey)$/i;
...
function visit(
  ...
  for (const [key, item] of entries.slice(0, options.maxObjectKeys)) {
    output[key] = SENSITIVE_FIELD_RE.test(key)
      ? REDACTED
